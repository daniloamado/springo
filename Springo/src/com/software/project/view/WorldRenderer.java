...
public abstract class WorldRenderer {
	...
	public void render() {
		...
		for (Platform p : platforms) {
			if (p instanceof TinyPlatform) {
				batch.draw(tinyPlatformTexture, p.getPosition().x, p.getPosition().y, 0, 0, p.getWidth() + 0.5f, p.getHeight() + 0.4f, scaleX, scaleY, p.getRotation(), 0, 0, tinyPlatformTexture.getWidth(), tinyPlatformTexture.getHeight(), false, false);
			} else {
				batch.draw(platformTexture, p.getPosition().x, p.getPosition().y, 0, 0, p.getWidth() + 0.5f, p.getHeight() + 0.4f, scaleX, scaleY, p.getRotation(), 0, 0, platformTexture.getWidth(), platformTexture.getHeight(), false, false);
			}
		}
